...
import '../../../data/model/response/home_request_model.dart';
...
abstract class _AuthStoreBase with Store {
  @observable
  BaseResponse<HomeRequestModel?>? loginResponse;
  @observable
...
bool isLoading=false;
  ...
  Future login(Map<String,dynamic> data) async {
    try {
      errorMessage = null;
      loginResponse = await authRepo.home(data);
...
subcategoryResponse=null;
    try {
      isLoading=true;
      errorMessage = null;
      subcategoryResponse = await authRepo.category(data);
      print("Data set hogaya");
    } on DioException catch (dioError) {
      errorMessage = DioExceptionUtil.handleError(dioError);
    } on AppException catch (e) {
      errorMessage = e.toString();
